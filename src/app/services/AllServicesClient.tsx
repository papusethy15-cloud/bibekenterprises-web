...
import { DomainService, DomainCategory, City, ServiceCityPrice, CustomerProfile } from "@/types";
...
export default function AllServicesClient({ services, categories, brand, cities, domainId }: Props) {
  ...
  // ── Category tabs ─────────────────────────────────────────────────────────
  const tabs = useMemo(
    () => categories.filter((cat) => services.some((s) => s.category_id === cat.category_id || s.category_name === cat.name)),
    [categories, services]
  );

  const filtered = useMemo(() => {
    if (activeCategory === "all") return services;
    return services.filter((s) => s.category_id === activeCategory || s.category_name === activeCategory);
  ...
  useEffect(() => {
    ...
    services.forEach((s) => fetchCityPrices(s.service_id));
  ...
  const handleBook = (svc: DomainService) => {
    if (!isLoggedIn) {
      window.location.href = `/login?redirect=${encodeURIComponent(`/services#book-${encodeURIComponent(svc.name)}`)}`;
      return;
    }
  ...
  useEffect(() => {
    ...
    if (match) {
      ...
      const svc = services.find((s) => s.domain_service_id === target) ?? services.find((s) => s.name === target);
      if (svc) { setPanel({ mode: "booking", service: svc }); window.history.replaceState(null, "", window.location.pathname); }
    }
  }, [isLoggedIn, hydrated, services]);
  ...
  const renderPanelBody = () => {
    ...
    if (!hydrated) {
      return (
        <div className="flex items-center justify-center h-full py-16">
          <div className="w-8 h-8 border-[3px] rounded-full animate-spin" style={{ borderColor: `${brand}30`, borderTopColor: brand }} />
        </div>
      );
    }
    ...
    if (panel.mode === "done") {
      return (
        <div className="flex flex-col items-center text-center gap-4 py-8">
          <div
            className="w-16 h-16 rounded-full flex items-center justify-center text-3xl animate-fade-in-up"
          ...
          <div
            className="rounded-xl px-6 py-3 w-full animate-fade-in-up"
            style={{ background: `${brand}0d`, border: `1px solid ${brand}30`, animationDelay: "160ms" }}
          >
    ...
    if (panel.mode === "booking" && isLoggedIn && customer) {
      return (
        <InlineBookingPanel
          ...
          cities={cities}
          domainId={domainId}
          onBookingDone={(bn) => setPanel({ mode: "done", bookingNumber: bn, serviceName: panel.service.name })}
        />
      );
    }
    ...
    // Idle — not logged in (guest)
    return (
      <div className="flex flex-col items-center text-center gap-5 py-8">
        <div className="w-16 h-16 rounded-full flex items-center justify-center text-3xl" style={{ background: `${brand}10` }}>
          👋
        </div>
        <div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 pb-20 pt-6">
        <div className="flex gap-6 items-start">
          ...
          <aside className="hidden md:block w-56 flex-shrink-0 sticky top-[88px]">
            <div className="bg-white rounded-2xl border border-ink-100 shadow-sm overflow-hidden">
              ...
              <nav className="py-2">
                ...
                <button
                  onClick={() => setActiveCategory("all")}
                  className="w-full text-left px-4 py-2.5 text-sm font-medium transition-all duration-200 flex items-center gap-2.5 group"
                  style={activeCategory === "all"
                    ? { background: `${brand}12`, color: brand, fontWeight: 700 }
                    : { color: "#4a4a54" }}
                >
                ...
                {tabs.map((cat) => {
                  const count = services.filter((s) => s.category_id === cat.category_id || s.category_name === cat.name).length;
                  ...
                  return (
                    ...
                    >
                      {isActive && (
                        <span className="absolute left-0 top-1 bottom-1 w-0.5 rounded-r-full" style={{ background: brand }} />
                      )}
            ...
            <div className="mt-4 bg-white rounded-2xl border border-ink-100 shadow-sm p-4 space-y-2 text-xs text-ink-500">
              ...
              <Link href="/customer/bookings" className="flex items-center gap-2 hover:text-ink-900 transition-colors py-1">
                📋 My Bookings
              </Link>
              <Link href="/customer/profile" className="flex items-center gap-2 hover:text-ink-900 transition-colors py-1">
                👤 My Profile
              </Link>
              <Link href="/#services" className="flex items-center gap-2 hover:text-ink-900 transition-colors py-1">
                🏠 Back to Home
              </Link>
            </div>
          </aside>
          ...
          {/* ── Col 2: Services grid ── */}
          <main ref={col2Ref} className="flex-1 min-w-0">
            {filtered.length === 0 ? (
          ...
          <aside id="right-panel" className="hidden lg:block w-72 flex-shrink-0 sticky top-[88px]">
            <div className="bg-white rounded-2xl border border-ink-100 shadow-sm overflow-hidden min-h-[460px] flex flex-col">
              {/* Panel header */}
              <div
                className="px-5 py-4 border-b border-ink-100 flex items-center justify-between"
                style={{ background: `linear-gradient(135deg, ${brand}08 0%, ${brand}14 100%)` }}
              >
                <div className="min-w-0">
                  {panel.mode === "booking" && (
                    <p className="text-[10px] text-ink-400 uppercase tracking-widest font-bold mb-0.5">Booking</p>
                  )}
                ...
                {panel.mode === "booking" && (
                  <button
                    onClick={() => setPanel({ mode: "idle" })}
                    className="ml-2 flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-ink-400 hover:text-ink-700 hover:bg-ink-100 transition-all"
                  >
                    ✕
                  </button>
                )}
      ...
      {panel.mode === "booking" && isLoggedIn && customer && (
        <div className="lg:hidden fixed inset-x-0 bottom-0 z-50 bg-white border-t border-ink-200 shadow-[0_-12px_40px_-8px_rgba(0,0,0,0.18)] rounded-t-3xl max-h-[85vh] overflow-y-auto animate-slide-up">
          <div
            className="flex items-center justify-between px-5 py-4 border-b border-ink-100 sticky top-0 bg-white z-10"
          >
            <div>
            ...
            <button
              onClick={() => setPanel({ mode: "idle" })}
              className="w-8 h-8 rounded-full bg-ink-100 flex items-center justify-center text-ink-500 hover:bg-ink-200 transition-colors font-bold"
            >
              ✕
            </button>
          </div>
      ...
      {panel.mode === "done" && (
        <div className="lg:hidden fixed inset-x-0 bottom-0 z-50 bg-white border-t border-ink-200 shadow-[0_-12px_40px_-8px_rgba(0,0,0,0.18)] rounded-t-3xl p-6 animate-slide-up">
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <button onClick={() => setPanel({ mode: "idle" })} className="text-sm font-semibold underline" style={{ color: brand }}>
              Book another
            </button>
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="flex-1 p-4 min-w-0">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-wrap gap-2 mt-2.5">
                  <span className="text-[10px] text-ink-500 bg-ink-50 border border-ink-100 rounded-full px-2 py-0.5 flex items-center gap-1">
              ...
              <div className="flex-shrink-0 text-right min-w-[80px]">
                ...
                <div className="text-xl font-extrabold text-ink-900 leading-tight">
            ...
            <div className="flex items-center gap-2 mt-4 pt-3.5 border-t border-ink-100">
              ...
              <Link
                ...
                style={{
                  borderColor: isHovered ? `${brand}60` : "#e5e5e8",
                  color: isHovered ? brand : "#6b7280",
                }}
              >
                Details →
              </Link>
        ...
        {isSelected && (
          <div className="h-0.5 w-full" style={{ background: `linear-gradient(90deg, ${brand}, ${brand}60)` }} />
        )}
      </div>
    </div>
  );
